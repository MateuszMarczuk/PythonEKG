...
record = wfdb.rdsamp('105')
sig, fields = wfdb.srdsamp('105', channels = [0])
...
Tree = pywt.wavedec(data=sig, wavelet='db2', level=4, mode='symmetric', axis=-2)


newTree = [Tree[0], Tree[1]*0, Tree[2]*0, Tree[3]*0, Tree[4]*0]

TreeHaar = pywt.wavedec(data=sig, wavelet='dmey', level=5, mode='symmetric', axis=-2)

for i in range(0, len(TreeHaar[0])):
    if TreeHaar[0][i] < 0.4:
        TreeHaar[0][i]=0

newTreedmey = [TreeHaar[0], TreeHaar[1]*0, TreeHaar[2]*0,TreeHaar[3]*0,TreeHaar[4]*0,TreeHaar[5]*0]


Tree3 = pywt.wavedec(data=sig, wavelet='coif1', level=4, mode='symmetric', axis=-2)

for i in range(0, len(Tree3[0])):
   if Tree3[0][i] < 0.4:
      Tree3[0][i]=0

newTreeCoif = [Tree3[0], Tree3[1]*0, Tree3[2]*0, Tree3[3]*0, Tree3[4]*0]
...
pyplot.title("Falka db2")
pyplot.show()
pyplot.figure(2)
pyplot.plot(recSignalHaar[:2000])
pyplot.title("Falka dmey")
pyplot.show()
pyplot.figure(3)
pyplot.plot(recSignal3[:2000])
pyplot.title("Falka coif2")
pyplot.show()
...
plikcsv="COIF1.MLII.BPM.105.csv"
with open(plikcsv, 'w', newline='') as file:
    x = csv.writer(file, delimiter=';')
...
print(wyniki(maximum(recSignal)))
print(wyniki(maximum(recSignalHaar)))
print(wyniki(maximum(recSignal3)))


plikcsv2="Wynik.DB2.MLII.csv"
with open(plikcsv2, 'w', newline='') as file:
    x = csv.writer(file, delimiter=';')
    x.writerows(wyniki(maximum(recSignal)))
    file.close()

plikcsv2="Wynik.DMEY.MLII.csv"
with open(plikcsv2, 'w', newline='') as file:
    x = csv.writer(file, delimiter=';')
    x.writerows(wyniki(maximum(recSignalHaar)))
    file.close()

plikcsv2="Wynik.COIF1.MLII.csv"
with open(plikcsv2, 'w', newline='') as file:
    x = csv.writer(file, delimiter=';')
    x.writerows(wyniki(maximum(recSignal3)))
    file.close()
